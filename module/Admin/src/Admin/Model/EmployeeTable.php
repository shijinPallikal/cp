...
class EmployeeTable extends AbstractTableGateway
{
    ...
    public function inserts(EmployeeModel $obj)
    {  
    ...
    {
        $sql="select * from tbl_employee where status=1 order by id DESC";
        $statement = $this->adapter->query($sql);           
        $result = $statement->execute(); 
        return $result;         
    }
    ...
    {
        $statement = $this->adapter->query("select * from tbl_employee where id= $id");
        $result = $statement->execute();
        return $result; 
    }
    public function updateData(EmployeeModel $obj)
    {  
        $sql = new Sql($this->adapter);         
    ...
    {
        $statement = $this->adapter->query("update tbl_employee set status= '0' where id=$id");
        $result = $statement->execute();
        return $result;
    }
    public function statusOn($id)
    {
        $statement = $this->adapter->query("update tbl_employee set status= '1' where id=$id");
        $result = $statement->execute();
        return $result;
    }
